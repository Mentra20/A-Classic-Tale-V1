...
void outro() {
    //génère desconfettis de couleur aléatoire et changeante, pour l’écran de fin
    image(FIN,0,0);
...
void bougerPerso() {
    //déplace lepersonnage selon la touche pressée, sélectionne une image plus sombre si il se trouve dans un terrain sombre
...
void PNJ() {
    if (terrain ==  3) {
        if (Gentleman ==  0) { image(gentleman,gentleX,gentleY); if (persoX ==  675 && persoY ==  450) {collB = 1;}
            ...
            if (text ==  2) {if (mouseX > 42 && mouseX < 852 && mouseY > 693 && mouseY < 770) {bulle = loadImage("bulle4.png");} elseif (mouseX > 42 && mouseX < 852 && mouseY > 799 && mouseY < 876) {bulle = loadImage("bulle5.png");}
